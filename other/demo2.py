...
import mpmath as mp
import numpy as np
from scipy.linalg import eig, eigh, eigvalsh, expm
...
def deriv_approx2(
    ...
    for j in range(np.max([0, N_a - 1]), N_b):
        ...
        t = np.dot(U.T, bkr)
        bkr = bkr - np.dot(U, t)
        phi, _ = scipy.sparse.linalg.gmres(matop, bkr, M=preop, tol=1e-10, atol=1e-15)
        phi = phi * eta[j]
...
def store_EG(fun, Gr, Kr, ks_rho, N_a, N_b):
    ...
    for i in range(n):
        for j in range(n):
            ...
            G[i, j] = Gij_ab(fun, ks_rho, eta_sum, mu[i], mu[j], lam_a, lam_b)
...
if __name__ == "__main__":
    # Set parameters
    rho = 100.0  # how large rho is depends on how close the eigenvalues are
    n = 100
    dh = 1e-8
    ndvs = 1
    ...
    for softmax in ["tanh", "exp"]:
        ...
        print("ans2 =", ans2)
        print("error2 =", np.abs(ans - ans2) / np.abs(ans))
