...
template <typename T>
py::array_t<T> assembleK(py::array_t<T> X_py, py::array_t<int> conn_py, py::array_t<T> rho_py,
  ...
  View3D<T> Ke = computeK<T>(X, conn, rho, C0, rho0_K, ptype_K, p, q);
...
// to compute the element stiffnesses
template <typename T>
py::array_t<T> assembleG(py::array_t<T> X_py, py::array_t<int> conn_py, py::array_t<T> rho_py,
                         py::array_t<T> u_py, py::array_t<T> C0_py, T rho0_K, const char* ptype_K,
                         double p, double q) {
  auto X = numpyArrayToView2D<T>(X_py);
  auto conn = numpyArrayToView2D<int>(conn_py);
  auto rho = numpyArrayToView1D<T>(rho_py);
